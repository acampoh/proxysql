...
*/
		{
			...
			int affected_rows=0;
			SQLite3_result *resultset=NULL;
			char *query = "SELECT hostgroup_id, hostname, port, gtid_port, CASE status WHEN 0 OR 1 OR 4 THEN 0 ELSE status END status, weight, compression, max_connections, max_replication_lag, use_ssl, max_latency_ms, comment FROM mysql_servers WHERE status<>3 ORDER BY hostgroup_id, hostname, port";
			mydb->execute_statement(query, &error , &cols , &affected_rows , &resultset);
			if (resultset) {
...
void MySQL_HostGroups_Manager::generate_mysql_replication_hostgroups_table() {
	...
	for (std::vector<SQLite3_row *>::iterator it = incoming_replication_hostgroups->rows.begin() ; it != incoming_replication_hostgroups->rows.end(); ++it) {
		...
		//}
		mydb->execute("INSERT INTO mysql_replication_hostgroups VALUES(%s,%s,'%s','%s')",r->fields[0], r->fields[1], r->fields[2], o);
...
void MySQL_HostGroups_Manager::read_only_action(char *hostname, int port, int read_only) {
	...
	mydb->execute_statement(Q1, &error , &cols , &affected_rows , &resultset, hostname, port);
	wrunlock();
	int num_rows=0;
	if (resultset==NULL) {
	...
	switch (read_only) {
		case 0:
			if (num_rows==0) {
				...
				if (GloMTH->variables.hostgroup_manager_verbose) {
					...
					admindb->execute_statement("SELECT * FROM mysql_servers WHERE hostname=\"%s\" AND port=%d", &error2 , &cols2 , &affected_rows2 , &resultset2, hostname, port);
					if (error2) {
						proxy_error("Error on read from mysql_servers : %s\n", error2);
					} else {
						if (resultset2) {
				...
				admindb->execute(Q2A,hostname,port);
				admindb->execute(Q2B,hostname,port);
				if (mysql_thread___monitor_writer_is_also_reader) {
					admindb->execute(Q3A,hostname,port);
				} else {
					admindb->execute(Q3B,hostname,port);
				}
				if (GloMTH->variables.hostgroup_manager_verbose) {
					char *error2=NULL;
					int cols2=0;
					...
					admindb->execute_statement(q, &error2 , &cols2 , &affected_rows2 , &resultset2, hostname, port);
					if (error2) {
						proxy_error("Error on read from mysql_servers : %s\n", error2);
					} else {
						if (resultset2) {
		...
		case 1:
			if (num_rows) {
				...
				admindb->execute(Q4,hostname,port);
				if (GloMTH->variables.hostgroup_manager_verbose) {
					char *error2=NULL;
					int cols2=0;
					int affected_rows2=0;
				...
				admindb->execute(Q5, hostname, port);
				if (GloMTH->variables.hostgroup_manager_verbose) {
					char *error2=NULL;
					int cols2=0;
					int affected_rows2=0;
...
void MySQL_HostGroups_Manager::update_group_replication_set_offline(char *_hostname, int _port, int _writer_hostgroup, char *_error) {
	...
	mydb->execute_statement(q, &error, &cols, &affected_rows, &resultset, _hostname, _port);
	if (error) {
		free(error);
		error=NULL;
	}
	...
	if (resultset) { // we lock only if needed
		if (resultset->rows_count) {
			...
			mydb->execute(q,_writer_hostgroup,_hostname,_port,_writer_hostgroup);
			...
			mydb->execute(q,_hostname,_port,_writer_hostgroup);
			...
			mydb->execute_statement(q, &error, &cols , &affected_rows , &resultset2,_writer_hostgroup);
			if (resultset2) {
				if (resultset2->rows_count) {
					for (std::vector<SQLite3_row *>::iterator it = resultset2->rows.begin() ; it != resultset2->rows.end(); ++it) {
						...
						mydb->execute(q, writer_hostgroup, backup_writer_hostgroup, reader_hostgroup, offline_hostgroup);
	...
	if (resultset) { // if we reach there, there is some action to perform
		if (resultset->rows_count) {
			...
			if (writer_is_also_reader && read_HG>=0) {
				...
				mydb->execute(q,read_HG,_writer_hostgroup,_hostname,_port);
			}
			converge_group_replication_config(_writer_hostgroup);
			commit();
			wrlock();
			...
			if (resultset2) {
				if (resultset2->rows_count) {
					for (std::vector<SQLite3_row *>::iterator it = resultset2->rows.begin() ; it != resultset2->rows.end(); ++it) {
						...
						q=(char *)"DELETE FROM mysql_servers WHERE hostgroup_id IN (%d , %d , %d , %d)";
						mydb->execute(q,_writer_hostgroup,backup_writer_hostgroup,reader_hostgroup,offline_hostgroup);
...
void MySQL_HostGroups_Manager::converge_group_replication_config(int _writer_hostgroup) {
	...
	if (it2!=Group_Replication_Info_Map.end()) {
		...
		mydb->execute_statement(q, &error, &cols, &affected_rows, &resultset, info->writer_hostgroup, info->backup_writer_hostgroup, info->reader_hostgroup, info->offline_hostgroup);
					...
					for (std::vector<SQLite3_row *>::reverse_iterator it = resultset->rows.rbegin() ; it != resultset->rows.rend(); ++it) {
						...
						if (to_move) {
							...
							if (hostgroup==info->writer_hostgroup) {
								...
								mydb->execute(q,info->backup_writer_hostgroup,info->writer_hostgroup,r->fields[1],atoi(r->fields[2]));
								to_move--;
							}
						}
					}
					...
					if (num_writers < info->max_writers && num_backup_writers) { // or way too low writer
						...
						for (std::vector<SQLite3_row *>::iterator it = resultset->rows.begin() ; it != resultset->rows.end(); ++it) {
							...
							if (to_move) {
								...
								if (hostgroup==info->backup_writer_hostgroup) {
									q=(char *)"UPDATE OR REPLACE mysql_servers_incoming SET status=0, hostgroup_id=%d WHERE hostgroup_id=%d AND hostname='%s' AND port=%d";
									mydb->execute(q,info->writer_hostgroup,info->backup_writer_hostgroup,r->fields[1],atoi(r->fields[2]));
									to_move--;
								}
							}
						}
			...
			if (resultset) {
				if (resultset->rows_count) {
					...
					if (num_backup_writers) { // there are backup writers, only these will be used as readers
						...
						mydb->execute(q, info->reader_hostgroup);
						...
						mydb->execute(q, info->reader_hostgroup, info->backup_writer_hostgroup);
					}
				}
				delete resultset;
				resultset=NULL;
...
void MySQL_HostGroups_Manager::update_galera_set_offline(char *_hostname, int _port, int _writer_hostgroup, char *_error) {
	...
	if (resultset) { // we lock only if needed
		if (resultset->rows_count) {
			...
			mydb->execute_statement(q, &error , &cols , &affected_rows , &numw_result, _hostname, _port);
		...
		if (set_offline) {
			...
			mydb->execute_statement(q1, &error, &cols, &affected_rows, &resultset_servers, _writer_hostgroup);
			if (error == NULL) {
			...
			mydb->execute_statement(q2, &error, &cols, &affected_rows, &resultset_servers, _writer_hostgroup);
			if (error == NULL) {
				if (resultset_servers) {
			...
			if (checksum_incoming!=checksum_current) {
				...
				mydb->execute_statement(q, &error, &cols , &affected_rows , &resultset2,_port,_writer_hostgroup);
				if (resultset2) {
					if (resultset2->rows_count) {
						for (std::vector<SQLite3_row *>::iterator it = resultset2->rows.begin() ; it != resultset2->rows.end(); ++it) {
							SQLite3_row *r=*it;
					...
					if (num_writers < info->max_writers && num_backup_writers) { // or way too low writer
						...
						if (num_writers == 0 && num_backup_writers == 0) {
							...
							if (pn->size()) {
								...
								mydb->execute_statement(q, &error , &cols , &affected_rows , &resultset2, info->writer_hostgroup, info->backup_writer_hostgroup, info->reader_hostgroup, info->offline_hostgroup);
											...
											for (it2=pn->begin(); (it2!=pn->end()) && !stop; ++it2) {
												...
												if (strcmp(h,host.c_str())==0) {
													if (p == port_n) {
														...
														mydb->execute(q,info->writer_hostgroup, info->writer_hostgroup, info->backup_writer_hostgroup, info->reader_hostgroup, info->offline_hostgroup, host.c_str(), port_n);

														int writer_is_also_reader = info->writer_is_also_reader;
														if (writer_is_also_reader) {
															int read_HG = info->reader_hostgroup;
															q=(char *)"INSERT OR IGNORE INTO mysql_servers_incoming (hostgroup_id,hostname,port,gtid_port,status,weight,compression,max_connections,max_replication_lag,use_ssl,max_latency_ms,comment) SELECT %d,hostname,port,gtid_port,status,weight,compression,max_connections,max_replication_lag,use_ssl,max_latency_ms,comment FROM mysql_servers_incoming WHERE hostgroup_id=%d AND hostname='%s' AND port=%d";
															mydb->execute(q,read_HG, info->writer_hostgroup, host.c_str(), port_n);
														}
													}
												}
											}
